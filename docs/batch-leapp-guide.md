...
Answering it needs three things per extraction: the list of installed apps, the list of every file each app owns, and the list of files the LEAPP modules actually touched. The first two now come from a developer-only artifact module (`appInventory.py`, shipped in `scripts/alternate_artifacts/` in both iLEAPP and ALEAPP, and regular users never see it). The third one the LEAPPs already record on every run. Batch LEAPP glues it all together:
